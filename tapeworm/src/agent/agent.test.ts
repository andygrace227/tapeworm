  ...
  it("runs returned tool calls and appends the tool output to the conversation", async () => {
    ...
    const responses = [
      Message.builder()
        ...
        .toolCalls([toolCall])
        .build(),
      Message.builder().role("assistant").content("done").build(),
    ];
    ...
    const toolMessage = agent.conversation.messages.find(
      (m: any) => m.role === "tool",
    );
    expect((toolMessage?.content[0] as ToolResult).toolName).toBe("echo");
    expect((toolMessage?.content[0] as ToolResult).toolResult).toEqual({
      received: "ping",
    });
